...
fs = 200e6;%200e6;%1e6;%0.5e6;%250e6;
N = 15;%127;%15;%31;
VDD = 1.2; % VDD of RDAC
...
for CCO_size_num = 1:5
    for CCO_load_num = 1:7
        for S = 1:4:17 %if fs is very small, we need very large S
...
disp("Based on range_valid_area_3, select the CCO type of 1st stage");

%for SRO, what matters are freq, thus, based on f_H and f_L

range_2nd = zeros(1,5);
range_valid_2nd = zeros(1,5);
count = 1;
count_valid = 1;
for CCO_size_num = 1:5
    for CCO_load_num = 1:7
        for S = 1%1:4:17
            ...
            range_2nd(count, :) = [CCO_size_num, CCO_load_num, S, area_CCO, freq];
            count = count + 1;
            %if (freq < 50e6)
                range_valid_2nd(count_valid, :) = [CCO_size_num, CCO_load_num, S, area_CCO, freq];
                count_valid = count_valid + 1;
            %end
        end
    end
...
range_valid_2nd_freq = sortrows(range_valid_2nd,5, "descend");

if (fs > 20e6)
    range_valid_2nd_freq_filt = range_valid_2nd_freq(range_valid_2nd_freq(:, 5)>0.4*fs & range_valid_2nd_freq(:, 5)<5*fs,:);
else
     range_valid_2nd_freq_filt_temp = range_valid_2nd_freq(range_valid_2nd_freq(:, 5)>0.4*fs,:); 
     range_valid_2nd_freq_filt = range_valid_2nd_freq_filt_temp(end-10:end, :);
end


range_valid_2nd_area = sortrows(range_valid_2nd_freq_filt,4, "ascend");

disp("Based on range_valid_2nd_area, select the CCO type of 2nd stage");
