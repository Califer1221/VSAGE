...
%BUF
area_BUF = (2*0.42 +2*0.56)*N*H;
area_BUF_act = area_BUF/factor;

%DAC_BUF
area_DAC_BUF = 0.42*N*2*H;
are_DAC_BUF_act = area_DAC_BUF/factor;

%digital
area_DIG = (3.08*2+0.28+2.24+0.7*2+0.42*2)*N*H;
area_DIG_act = area_DIG/factor;

%DAC
%2.3 is to give DAC larger area
s_dac = res_gen(RDAC_u);
area_DAC = dot(RES_lib_size, s_dac)* N * H*2.3;
